...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      return createElement(
        'button',
        {
          ...
          style: {
            ...
            padding: '5px 16px',
            borderRadius: '10px',
            border: '1px solid var(--dsw-alias-border-l1, #d8dde3)',
            background: '#ffffff',
    ...
    /**
     * Quote chip INSIDE the input box: registered in the input dock row
     * (reliably provides inputActions + the live InputState snapshot) but
     * rendered as a fixed-positioned chip pinned just above the composer
     * textarea via translateY(-100%) — whatever its own height, the chip sits
     * inside the composer card, directly above the text area. Also wraps
     * `submit` to inject the blockquote on send, and doubles as the session
     * bridge (forwards inputActions + input to the floating selection button).
     */
      ...
      useEffect(() => {
        ...
        const measure = () => {
          try {
            ...
            setPos({ left: r.left, top: r.top - 12, width: r.width })
          } catch {
            setPos(null)
          }
        }
    ...
    function apply(ctx) {
      ...
      //    conversation.input.dock reliably provides inputActions + the live
      //    input snapshot; the chip itself is fixed-positioned over the
      //    composer textarea (see QuoteCard).
      ctx.effect(() =>
