...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Quote chip INSIDE the input box: rendered from the InputBar floating
     * overlay anchor (session scope) and pinned just above the composer
     * textarea via fixed positioning + translateY(-100%) — whatever its own
     * height, the chip sits inside the composer card, directly above the text
     * area. Also wraps `submit` to inject the blockquote on send, and doubles
     * as the session bridge (forwards inputActions + input to the floating
     * selection button).
     */
    function QuoteCard({ inputActions, useInput }) {
    ...
    function apply(ctx) {
      ...
      ctx.effect(() =>
        slots.inject('conversation.input.overlay', () =>
          slots.register(
            { name: 'conversation.input.overlay', id: 'dsh-quote-card' },
            (props) =>
