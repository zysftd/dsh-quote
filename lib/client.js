...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const shared = { inputActions: null, useInput: null }
    ...
    function SessionBridge({ inputActions, useInput }) {
      useEffect(() => {
        ...
        shared.useInput = useInput
        return () => {
          if (shared.inputActions === inputActions) {
            shared.inputActions = null
            shared.useInput = null
          }
        }
      }, [inputActions, useInput])
      return null
    }
    ...
    function FloatQuote() {
      ...
      const onClick = (e) => {
        ...
        e.stopPropagation()
        try {
          const actions = shared.inputActions
          if (!actions) return
          const state = shared.useInput ? shared.useInput() : null
          const draft = (state && state.draft) || ''
          actions.setDraft(composeDraft(buildQuote(mark.text), draft))
          const s = window.getSelection()
