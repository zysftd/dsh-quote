...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function QuoteCard({ inputActions, input }) {
      ...
      // Placeholder strip INSIDE the input box: reserve real space at the top
      // of the composer textarea (padding-top) so the quote pill sits on an
      // empty placeholder line and never covers the typed text. A short
      // interval defends against React resetting the inline padding and keeps
      // the pill glued to the correct textarea (the composer one is the
      // largest / matches the current draft).
      useEffect(() => {
        if (typeof document === 'undefined' || typeof window === 'undefined') return undefined
        let timer = null
        const findTa = () => {
        ...
        const apply = () => {
          try {
            ...
            if (pending) {
              ta.style.paddingTop = '34px'
              const r = ta.getBoundingClientRect()
              if (r.width === 0 && r.height === 0) {
                setPos(null)
                return
              }
              setPos({ left: r.left + 8, top: r.top + 8 })
            } else {
              ta.style.paddingTop = ''
              setPos(null)
            }
