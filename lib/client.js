...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // The --dsw-alias-* tokens are defined on <body> (dark via
    // body[data-ds-dark-theme]), so resolve them from the live stylesheet —
    // body first, then <html>, then the composer textarea's ancestors — and
    // skip any value that is still an unresolved var() chain.
    const LIGHT = {
      bg: '#ffffff',
      fg: '#1f2328',
      muted: '#57606a',
    ...
    function resolveTheme() {
      try {
        ...
        while (el && roots.length < 8) {
          roots.push(el)
          el = el.parentElement
        }
        const pick = (name, fb) => {
          for (const r of roots) {
            const v = window.getComputedStyle(r).getPropertyValue(name).trim()
            if (v && v !== 'initial' && v !== 'inherit' && v !== 'unset' && !v.startsWith('var(')) {
              return v
            }
          }
          return fb
    ...
    function apply(ctx) {
      ...
      // switches at runtime (plus a MutationObserver fallback on <html>/<body>).
      try {
      ...
      try {
        if (typeof MutationObserver !== 'undefined' && typeof document !== 'undefined') {
          ...
          if (document.documentElement) ob.observe(document.documentElement, { attributes: true })
          if (document.body) ob.observe(document.body, { attributes: true })
