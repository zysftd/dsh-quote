...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function QuoteCard({ inputActions, input }) {
      ...
      // Embed the chip INSIDE the input box: push the textarea content down
      // with a temporary padding-top and overlay the chip on the freed strip
      // at the top of the textarea. This never overlaps the dock rows above
      // (todo/goal/queue) and never covers the user's typed text.
      useEffect(() => {
        if (typeof document === 'undefined' || typeof window === 'undefined') return undefined
        const apply = () => {
          try {
            ...
            if (pending) {
              ta.style.paddingTop = '46px'
              const r = ta.getBoundingClientRect()
              if (r.width === 0 && r.height === 0) {
                setPos(null)
                return
              }
              setPos({ left: r.left + 8, top: r.top + 8, width: r.width - 16 })
            } else {
              ta.style.paddingTop = ''
              setPos(null)
            }
      ...
      const display = pending.text.length > 120 ? pending.text.slice(0, 120) + '…' : pending.text
