...
//
// No host half, no network, no persistence: the draft is the only thing
// written, and only when the user clicks.
...
function extractMessageText(session, messageId) {
  try {
    ...
    return node.data.blocks
      .filter((b) => b && (b.kind === 'text' || b.kind === 'reasoning') && typeof b.text === 'string')
      .map((b) => b.text)
      .join('\n\n')
  } catch {
    return ''
...
/**
 * Floating selection quote button. Rendered into the InputBar overlay anchor
 * (session scope), so it receives `inputActions` + `useInput` standard props.
 * It renders nothing while there is no usable selection.
 */
  ...
  useEffect(() => {
    ...
    const update = () => {
      try {
        ...
        // quote button) or from inside the sidebar/settings chrome.
...
/**
 * Per-message "引用" action on finalized assistant messages: quotes the whole
 * message body into the composer.
 */
